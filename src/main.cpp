...
void checkFlyingState(uint32_t tAct)
{
  ...
  {
    //on ground
    if ((status.GPS_Fix) && (status.GPS_NumSat >= 4) && (status.GPS_speed > MIN_FLIGHT_SPEED))
    { //minimum 4 satelites to get accurade position and speed
...
void sendAWGroundStationdata(uint32_t tAct)
{
  ...
  if ((tAct - tSend) < 300000)
    return; //every 5min.
  if (!setting.awLiveTracking)
    return;
  tSend = tAct;
  char chs[20];
  String msg = ">GS Location," + setting.myDevId + ",";
...
void sendTraccarTrackingdata(FanetLora::trackingData *FanetData)
{
  ...
  if ((!setting.traccarLiveTracking) || (!setting.TraccarSrv.startsWith("http://")) || (WiFi.status() != WL_CONNECTED) || (!status.bTimeOk))
    return;
...
#endif
  msg += setting.myDevId + "," + fanet.getDevId(FanetData->devId) + "," + String((uint8_t)FanetData->aircraftType) + ",";
  sprintf(chs, "%02.6f", FanetData->lat);
  msg += String(chs) + ",";
  sprintf(chs, "%02.6f", FanetData->lon);
  msg += String(chs) + ",0,0,";
...
void sendData2Client(String data)
{
  if (setting.outputMode == OUTPUT_UDP)
  {
    ...
    { //connected to wifi or a client is connected to me
      //log_i("sending udp");
      WiFiUDP udp;
  ...
  if ((setting.outputMode == OUTPUT_SERIAL) || (setting.bOutputSerial))
  { //output over serial-connection
    Serial.print(data);
  }
#ifdef BLUETOOTHSERIAL
  if (setting.outputMode == OUTPUT_BLUETOOTH)
  { //output over bluetooth serial
...
esp_sleep_wakeup_cause_t print_wakeup_reason()
{
  ...
  case ESP_SLEEP_WAKEUP_UNDEFINED:
    log_i("wakeup undefined --> possible by reset");
    break;
...
#ifdef XCTRACER
void notifyXCTracerCallback(NimBLERemoteCharacteristic *pBLERemoteCharacteristic, uint8_t *pData, size_t length, bool isNotify)
{
  if (isNotify)
  {
...
void taskXCTracer(void *pvParameters)
{
  ...
  log_i("start xctracer task");
  BLE_FIFO_RX = new cbuf(256);
...
void taskStandard(void *pvParameters)
{
  ...
  if (setting.band == BAND915)
    frequency = FREQUENCY915;
  ...
  while (1)
  {
    ...
    tLoop = tAct;
    if (status.tMaxLoop < status.tLoop)
      status.tMaxLoop = status.tLoop;
...
#endif
        status.GPS_Fix = 1;
        if (status.GPS_NumSat < 4)
        { //we need at least 4 satellites to get accurate position !!
          status.GPS_speed = 0;
          status.GPS_course = 0;
        }
        else
    ...
    if (AXP192_Irq)
    {
      if (axp.readIRQ() == AXP_PASS)
      {
        ...
        if (axp.isPEKShortPressIRQ())
        {
          ...
          if (setting.screenNumber > MAXSCREENS)
...
void powerOff()
{
  ...
  */
  ...
  while (1)
  {
    ...
    if (xHandleBaro != NULL)
      tBaro = eTaskGetState(xHandleBaro);
    if (xHandleEInk != NULL)
      tEInk = eTaskGetState(xHandleEInk);
    if (xHandleStandard != NULL)
      tStandard = eTaskGetState(xHandleStandard);
    if (xHandleWeather != NULL)
      tWeather = eTaskGetState(xHandleWeather);
    if ((tBaro == eDeleted) && (tEInk == eDeleted) && (tWeather == eDeleted) && (tStandard == eDeleted))
      break; //now all tasks are stopped
...
void taskBackGround(void *pvParameters)
{
  ...
  while (1)
  {
    ...
    {
      if (timeOver(tAct, tRuntime, 180000))
    ...
    if ((status.vBatt < BATTEMPTY) && (status.vBatt >= BATTPINOK))
    { // if Batt-voltage is below 1V, maybe the resistor is missing.
      if (timeOver(tAct, tBattEmpty, 60000))
      { //min 60sek. below
